...
class EdidReader {
  ...
  getLinuxSystemEdids() {
    ...
    return glob('/sys/devices/**/edid').map(edidFileName => fs.readFileAsync(edidFileName)).map(buffer => buffer.toString('hex')).filter(edid => edid !== '');
  }
  ...
  // Scan host for edids
  scan() {
    return this.getSystemEdids().map(this.formatEdid).then(rawEdids => {
  ...
  static parse(rawEdid) {
    ...
    edidParser.parse();
    var manufacturer = EdidReader.eisaIds[edidParser.eisaId] || { name: '', fullName: '' };
    edidParser.manufacturer = manufacturer.name;
    edidParser.manufacturerFullName = manufacturer.fullName;
    return edidParser;
  }
...
EdidReader.eisaIds = require(`${ __dirname }/../data/eisa.json`);
